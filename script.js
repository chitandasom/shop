...
let dragStartX = 0;
let dragStartY = 0;
let dragMoved = false;  // ⭐ 실제로 움직였는지 판단하는 핵심 변수
let cancelNextClick = false;
let topZIndex = 10; // ⭐ 카드 쌓임 순서 관리용
...
function renderMobile(filtered) {
  ...
  filtered.forEach(prod => {
    ...
    card.dataset.id = prod.id;   // ⭐ 상세페이지 이동용 ID
    ...
    if (prod.outOfStock) {
      info.innerHTML = `
    ${prod.name}<br>
    <span class="soldout-price">${prod.price}</span>
    &nbsp;out of stock
  `;
    } else {
...
function renderWeb(filtered) {
  ...
  filtered.forEach(prod => {
    ...
    const priceHTML = prod.outOfStock
      ? `<span class="soldout-price">${prod.price}</span> out of stock`
      : prod.price;
...
function enableMobileDrag() {
  ...
  cards.forEach(card => {
    ...
    const LONG_PRESS_TIME = 200; // 200ms 이상 눌러야 드래그 시작

    function startDrag(x, y) {
      isDragging = true;  // ⭐ 터치 드래그도 클릭 막기
      dragging = true;

      topZIndex++;
      card.style.zIndex = topZIndex; // ⭐ 모바일도 동일

      document.body.classList.add("noscroll");   // ⭐ 스크롤 잠금
    ...
    function moveDrag(x, y) {
      ...
      let newY = cardY + deltaY + scrollDiffY;
    ...
    function endDrag() {
      ...
      if (dragMoved) {
        cancelNextClick = true;   // ⭐ 움직였을 때만
      }

      dragMoved = false;          // ⭐ 리셋해줘야 클릭 가능!
      dragging = false;
      isDragging = false;
